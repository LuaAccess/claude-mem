...
- **Dependabot:** Enabled for automatic security updates (added in LuaAccess/claude-mem hardening fork)
- **Manual Review:** Critical dependencies reviewed quarterly


## Security Hardening (LuaAccess Fork)

The following changes were applied to this fork beyond the upstream baseline:

| Change | File | Reason |
|---|---|---|
| Telemetry default changed to opt-in (OFF) | `src/services/telemetry/consent.ts` | Upstream default was opt-out (ON without consent) |
| AWS/GCP credentials removed from subprocess env | `src/supervisor/env-sanitizer.ts` | Prevents credential propagation to subprocesses |
| cmem.ai waitlist endpoint disabled | `src/npx-cli/commands/install.ts` | Removes external data submission during install |
| `chmod 600` added to settings file write | `openclaw/install.sh` | Protects API keys stored in settings.json |
| All GitHub Actions SHA-pinned | `.github/workflows/*.yml` | Prevents supply chain compromise via floating tags |
| Dependabot added | `.github/dependabot.yml` | Automated dependency vulnerability scanning |

To disable telemetry in this fork (belt-and-suspenders):
```bash
