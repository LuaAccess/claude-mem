...
// HARDENED: submitOnlineSignup disabled. Original code POSTed email + platform
// metadata to https://cmem.ai/api/waitlist. Stubbed to no-op for privacy.
// eslint-disable-next-line @typescript-eslint/no-unused-vars
async function submitOnlineSignup(_payload: { email: string; note: string; version: string }): Promise<boolean> {
  return false; // HARDENED: external signup endpoint disabled
}
...
// HARDENED: promptCmemOnlineOptIn disabled. Original code collected email and
// sent it to cmem.ai/api/waitlist along with version and platform metadata.
// eslint-disable-next-line @typescript-eslint/no-unused-vars
async function promptCmemOnlineOptIn(_version: string): Promise<void> {
  return; // HARDENED: cmem.ai online signup prompt disabled
  // Original guard conditions preserved below for reference:
  if (!isInteractive) return;
  if (process.env.CI) return;
