...
export type TelemetryConfig = {
  /** Explicit user decision. Absent = no decision recorded; the opt-in default applies (telemetry OFF until enabled). */  // HARDENED
  enabled?: boolean;
  installId: string;
  decidedAt: string;
};
 ...
 * 4. Default: off (opt-in — telemetry disabled until explicitly enabled) // HARDENED
 */
export function explainTelemetryConsent(
  ...
  return { enabled: false, source: 'default' }; // HARDENED: changed from opt-out to opt-in default
}
