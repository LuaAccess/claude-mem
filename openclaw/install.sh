...
install_bun() {
  ...
  # HARDENED: Note — this pipes a remote script to bash without SHA verification.
  # For maximum security, install Bun manually: https://bun.sh
  # or via your package manager: brew install oven-sh/bun/bun
...
install_uv() {
  ...
  # HARDENED: Note — this pipes a remote script to sh without SHA verification.
  # For maximum security, install uv manually: https://docs.astral.sh/uv/getting-started/installation/
  # or via your package manager: brew install uv
...
write_settings() {
  ...
  # HARDENED: lock down settings file permissions — contains API keys
  chmod 600 "${settings_file}" 2>/dev/null || warn "Could not set permissions on ${settings_file} — run: chmod 600 ${settings_file}"
  success "Settings written to ${settings_file} (permissions: 600)"
}
